...
// lowest-mastery heuristic (ported inline — the original imports the React
// config). Renders Problem; handles graduation / exhaustion. Route: /lessons/:id
...
const mastery = ref(0)
const lessonProblems = ref<any[]>([])
const bkt = reactive<Record<string, any>>(JSON.parse(JSON.stringify(bktParamsJson)))
const completed = reactive(new Set<string>())
const currProblem = ref<any>(null)
const seed = ref(Date.now())

const lesson = computed(() => {
  const id = route.params.id as string
  for (const c of coursePlans as any[]) {
    const l = (c.lessons || []).find((x: any) => x.id === id)
    if (l) return { ...l, courseName: c.courseName }
  }
  return null
})

/** Lowest-mastery heuristic (port of defaultHeuristic). */
function heuristic(problems: any[]): any | undefined {
  const chosen: any[] = []
  for (const p of problems) {
    ...
    if (chosen.length === 0 || chosen[0].probMastery > p.probMastery)
      chosen.splice(0, chosen.length, p)
...
onMounted(async () => {
  ...
  const mine = pool.filter(p => p.lessonId === lesson.value!.id && p.steps?.length)
